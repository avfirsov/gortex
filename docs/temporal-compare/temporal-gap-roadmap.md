...
> This document describes gaps found in a Temporal dispatch resolution fork. Repo names and identifiers have been abstracted; actual results will vary by codebase.
>
...
**Результат на наших репо:** 52 resolved dispatch (из 332 OFF), 57 temporal-stub edges. Но 186 новых broken_dispatch и 37 orphan-сущностей — области, которые форк видит, но не раскрывает.
...
```go
// workflow-repo-alpha/workflow/env_dispatch_workflow.go:18
activity := envhelper.GetEnvOrDefault(SERVICE_ACTIVITY_NAME_ENV, SERVICE_ACTIVITY_NAME_DEFAULT)
// ...
workflow.ExecuteActivity(ctx, activity, in)
```

Форк помечает такие stub-ы `temporal_name_origin=env_default` и ставит `temporalEnvDefaultConfidence = 0.4` (speculative), **но только если экстрактор уже вытащил имя**. В нашем случае экстрактор Go (`internal/parser/languages/golang_temporal.go`) **не раскрывает** `envhelper.GetEnvOrDefault` — он видит bare identifier `activity` и не может определить, что это env-fallback с конкретным дефолтом.

### Масштаб

8+ сайтов в `workflow-repo-alpha/workflow/env_dispatch_workflow.go` + `env_dispatch_workflow_v2.go`, аналогичные в `workflow-repo-beta/workflow/call*.go`.
...
`workflow-repo-alpha` dispatch-сайты (`callCreateServiceActivity`, `callPatchServiceActivity` и т.д.) больше не в `broken_dispatch`. В `synthesizers --temporal on` появляются temporal-stub edges от `OrderHandlingWorkflow` к `CreateServiceActivity`, `PatchServiceActivity` и т.д. с `via=temporal.stub` и `temporal_name_origin=env_default`.
...
```go
// workflow-repo-gamma/activity_executor/activity_name_resolver.go:21
workflow.ExecuteActivity(ctx, constants.GetCreateActivityName, in)
```

где `GetCreateActivityName` — это `func() string { return "CreateAcrmCasesActivity" }`.
...
`workflow-repo-gamma/activity_executor/*.go` — 10+ файлов с `constants.Get*ActivityName` / `constants.Get*ActivityQueue`. Аналогичный паттерн в `workflow-repo-eta`, `service-repo-delta` и др.
...
`GetCreateActivityName` в `workflow-repo-gamma/activity_executor/activity_name_resolver.go:21` больше не в `broken_dispatch` под `--temporal on`. В `synthesizers` появляется edge от `CreateAcrmCasesActivity` к `CreateAcrmCasesActivity` (или зарегистрированному имени) с `temporal_const_value="CreateAcrmCasesActivity"`.
...
```go
// workflow-repo-delta/lifecycle_workflow.go:129
...
3+ сайта в `workflow-repo-delta`, `ReceiveContractDocumentActivity` в `workflow-repo-epsilon`.
...
`RetrieveCartActivity` и `DeleteCartActivity` в `workflow-repo-delta` больше не в `broken_dispatch` под `--temporal on`. В `synthesizers` появляются edges от lifecycle workflow к соответствующим activity-функциям в `activities-repo-alpha/` (или аналогичном).
...
Форк реализует `resolveTemporalSignalQueryLinks` — связывает `SignalExternalWorkflow(ctx, ..., "save-state-signal", ...)` с `workflow.SetSignalHandler(ctx, "save-state-signal", ...)`. Но на наших репо signal остаётся в `signal_no_handler`:

```
signal "save-state-signal" → from=signalParentToSaveState @ workflow-repo-gamma/steps/api/workflow_step.go:112
```
...
1 signal в `workflow-repo-gamma`. Java-сервисы также используют signals/queries (но Java репо пока не в индексе).

### Решение

1. **Убедиться что extractor Go emit-ит `temporal.handler` edges** для `SetSignalHandler` / `GetSignalChannel`. Проверить: если extractor не видит `SetSignalHandler(ctx, "save-state-signal", handler)`, то `resolveTemporalSignalQueryLinks` не найдёт provider.
...
`signal_no_handler` count → 0 под `--temporal on`. В `synthesizers` появляются edges от `signalParentToSaveState` к workflow с `SetSignalHandler(ctx, "save-state-signal", ...)`.
...
```go
// workflow-repo-zeta/workflow/validation_workflow.go:399
future := DispatchByMode(ctx, activityOptions, constant.GetGetAllowedWithV2ActivityName, ...)
```

где `DispatchByMode` вызывает `ExecuteActivity(ctx, name, ...)` — но dispatcher видит только вызов `DispatchByMode`, а не `ExecuteActivity` внутри неё.

Аналогично:

```go
// workflow-repo-eta/businesslogic/validation_logic.go:839
workflow.ExecuteActivity(ctx, GET_QUALIFICATION_ACTIVITY, ...)
```

где `GET_QUALIFICATION_ACTIVITY` — const, которая **не попала** в `constVal` index (возможно, определена в другом файле через `func()`).

### Масштаб

10+ сайтов в `workflow-repo-zeta`, `workflow-repo-eta`, `workflow-repo-theta`.
...
**Альтернатива**: при анализе `DispatchByMode` — если функция находится в **другом пакете** и её тело содержит `ExecuteActivity(ctx, <param>, ...)`, extractor уже должен emit-ить `temporal_name_param` для этой функции. Проверить — если нет, это extractor bug.
...
- `internal/parser/languages/golang_temporal.go` — проверить что cross-package wrapper functions (типа `internal/envhelpers.ExecuteActivityHelper`) получают `temporal_name_param`

### Критерий приёмки

`GET_QUALIFICATION_ACTIVITY` в `workflow-repo-eta` не в `broken_dispatch`. `DispatchByMode` вызовы резолвятся к целевым activity. В `synthesizers` появляются edges для ранее unresolved dispatch-ов.
...
3. **Extractor — bottleneck**. Большинство gap-ов происходят не потому что resolver не умеет, а потому что **extractor не emit-ит нужные meta**. Конкретно:
   - `envhelper.GetEnvOrDefault` → `temporal_name_origin=env_default` — extractor не видит
   - `constants.GetFooName()` → `temporal_name=FooActivity` — extractor не раскрывает
   - `DispatchByMode(ctx, ao, name, ...)` → `temporal_name_param` для cross-package wrapper
