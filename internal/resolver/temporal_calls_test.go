...
// repo resolves, via the convention index, to an activity function that lives
// only in a *_test.go fixture of an UNRELATED workflow repo. That target is a
// test mock, not the real implementation, so the edge is spurious. The
// resolver must leave the stub unresolved rather than mint the cross-repo
// test-stub edge.
func TestResolveTemporalCalls_CrossRepoTestStubSuppressed(t *testing.T) {
	...
	// The only candidate named GetCatalogActivity is a test stub in a
	// different repo's *_test.go — a mock, not the real activity.
...
// filter from over-reaching: a test workflow dispatching a convention-named
// activity that lives in the SAME repo's *_test.go is the overwhelmingly
// common (and correct) case — it must still resolve.
func TestResolveTemporalCalls_SameRepoTestActivityResolves(t *testing.T) {
	...
	assert.Equal(t, activity.ID, call.To, "same-repo *_test.go activity must still resolve via convention")
}
