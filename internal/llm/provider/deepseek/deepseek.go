...
func (p *Provider) Complete(ctx context.Context, req llm.CompletionRequest) (llm.CompletionResponse, error) {
	...
	text, err := httpx.Complete(ctx, "deepseek", func(ctx context.Context) httpx.Result {
...
func (p *Provider) attempt(ctx context.Context, raw []byte) httpx.Result {
	...
	if err != nil {
		return httpx.Result{Err: fmt.Errorf("deepseek: build request: %w", err)}
	}
	...
	if err != nil {
		return httpx.Result{Err: fmt.Errorf("deepseek: request failed: %w", err)}
	}
	defer resp.Body.Close()
	payload, err := io.ReadAll(io.LimitReader(resp.Body, 4<<20))
	if err != nil {
		return httpx.Result{Err: fmt.Errorf("deepseek: read response: %w", err)}
	}

	var parsed apiResponse
	if err := json.Unmarshal(payload, &parsed); err != nil {
		return httpx.Result{Err: fmt.Errorf("deepseek: decode response (status %d): %w", resp.StatusCode, err)}
	}
	if resp.StatusCode != http.StatusOK {
		if parsed.Error != nil {
			return httpx.Result{Err: fmt.Errorf("deepseek: API error (status %d): %s: %s", resp.StatusCode, parsed.Error.Type, parsed.Error.Message)}
		}
		return httpx.Result{Err: fmt.Errorf("deepseek: API error (status %d): %s", resp.StatusCode, snippet(payload))}
	}
