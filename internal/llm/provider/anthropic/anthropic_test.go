...
// whose content blocks carry no text is treated as a transient
// truncation: the first hollow answer is retried and the second, good
// answer wins.
func TestComplete_HollowResponseRetriesThenSucceeds(t *testing.T) {
	...
	srv := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		_, _ = io.WriteString(w, `{"content":[{"type":"text","text":"recovered"}]}`)
	}))
	defer srv.Close()
