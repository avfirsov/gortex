...
func testMergeNodeMeta(t *testing.T, factory Factory) {
	...
	if got == nil || got.Meta["ext_summary"] != "computes foo" || got.Meta["ext_domain"] != "core" {
	...
	changed, found = s.MergeNodeMeta("a.go::Foo", map[string]any{"ext_summary": "revised"})
	if !found || !changed {
		t.Fatalf("overwrite merge: changed=%v found=%v, want true/true", changed, found)
	}
	got = s.GetNode("a.go::Foo")
	if got.Meta["ext_summary"] != "revised" || got.Meta["ext_domain"] != "core" {
	...
	changed, found = s.MergeNodeMeta("a.go::Ghost", map[string]any{"ext_summary": "x"})
	if found || changed {
	...
	changed, found = s.MergeNodeMeta("b.go::Bar", map[string]any{"ext_summary": "lazy"})
	if !found || !changed {
		t.Fatalf("lazy-init merge: changed=%v found=%v, want true/true", changed, found)
	}
	if s.GetNode("b.go::Bar").Meta["ext_summary"] != "lazy" {
