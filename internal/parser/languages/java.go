...
type JavaExtractor struct {
	...
	// detector (corporate, per-repo, loaded from the git-ignored
	// `.gortex/temporal-allowlist.yaml`). javaInvokers holds invoker class
	// simple-names; javaInvokerMethods the dispatch method names (defaults to
	// goJavaInvokerDefaultMethods when nil). Empty javaInvokers → detection OFF.
...
type javaDeferredCall struct {
	...
	isSelector bool
	callNode   *sitter.Node // the method_invocation node, for argument inspection
}
