...
// detector is OFF until configured (`java_temporal_invokers` in the git-ignored
// `.gortex/temporal-allowlist.yaml`). Precision-first: a call whose receiver
// type can't be resolved to a configured invoker, or whose name can't be
// extracted, is left to the generic call path (false negatives acceptable,
// false positives not).
//
